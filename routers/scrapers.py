...
async def trigger_scraper(
    ...
    if request.source.value in ["thingiverse", "ravelry", "github", "goat"]:
        config_response = (
            ...
            .select("access_token")
            .eq("platform", request.source.value)
            .execute()
        )
...
async def load_url(
    ...
    try:
        ...
        supported_sources = (
            supported_sources_response.data if supported_sources_response.data else []
        )
    except Exception as e:
    ...
    if not supported_sources:
        raise HTTPException(status_code=400, detail="No supported sources are configured yet.")
...
async def oauth_callback(
    ...
    try:
        ...
        if not token_data.get("access_token"):
            ...
            raise HTTPException(
                status_code=502,
                detail=f"OAuth token exchange returned no access token for {platform}",
            )
        ...
        raise HTTPException(
            status_code=400, detail=f"OAuth token exchange failed for {platform}: {response_text}"
        )
    except Exception as exc:
        logger.error("Failed to persist OAuth token for %s", platform, exc_info=True)
        raise HTTPException(
            status_code=500, detail=f"Failed to save OAuth token for {platform}: {str(exc)}"
        )
...
async def save_oauth_token(
    ...
    logger.info(
        f"save_oauth_token called with platform={platform}, token_data keys={list(token_data.keys())}"
    )
    ...
    try:
        ...
        if config_response.data:
            ...
            logger.info(
                f"Updating existing config for {platform} with fields: {list(update_data.keys())}"
            )
...
async def get_oauth_config(
    ...
    return {**config, "has_access_token": has_token}
...
async def get_oauth_configs(
    ...
    response = (
        db.table("oauth_configs")
        .select("id,platform,client_id,redirect_uri,created_at,updated_at")
        .execute()
    )
    return response.data
    ...
    if existing.data:
        ...
        create_data = {
            ...
            **{
                ...
                if k not in ("client_id", "client_secret", "redirect_uri")
            },
        }
...
async def get_search_terms(
    ...
    fallback = (
        GitHubScraper.SEARCH_TERMS
        if platform == "github"
        else (
            ThingiverseScraper.SEARCH_TERMS
            if platform == "thingiverse"
            else RavelryScraper.PA_CATEGORIES
        )
    )
...
async def update_search_terms(
    ...
    for term in request.search_terms:
        ...
        if len(term) > 100:
            raise HTTPException(
                status_code=400, detail="Search terms must be 100 characters or less"
            )
...
async def legacy_get_github_search_terms(
...
async def legacy_update_github_search_terms(
    request: UpdateSearchTermsRequest,
    current_user: dict = Depends(get_current_user),
    db=Depends(get_db),
):

...
def _canonicalize_source_value_db(db, value: str | None) -> str | None:
    if value is None:
        return None
...
async def _enrich_manual_product_metadata(
    db, source_name: str, source_url: str, product: ProductCreate, db_data: dict
):
    ...
    def _get_token(platform: str, env_keys: list[str]) -> str | None:
        token = None
        try:
            resp = (
    ...
    scraper_map = {
        "thingiverse": {
            ...
            "factory": lambda token: __import__(
                "scrapers.thingiverse", fromlist=["ThingiverseScraper"]
            ).ThingiverseScraper(db, token),
        ...
        "github": {
            "platform": "github",
            "factory": lambda token: __import__(
                "scrapers.github", fromlist=["GitHubScraper"]
            ).GitHubScraper(db, token),
        ...
        "ravelry": {
            "platform": "ravelry",
            "factory": lambda token: __import__(
                "scrapers.ravelry", fromlist=["RavelryScraper"]
            ).RavelryScraper(db, token),
    ...
    try:
        ...
        try:
            ...
            logger = logging.getLogger(__name__)
            logger.error("Exception closing scraper", exc_info=True)
            pass
...
async def get_product_sources(
    ...
    try:
        ...
        for source in canonical_set:
            result.append({"name": source, "count": source_counts.get(source, 0)})
...
def attach_rating_fields(db, product: dict, ratings_map: dict[str, dict] | None = None) -> dict:
...
async def get_tags(
    ...
    source: list[str] | None = Query(
        None, alias="source", description="Filter tags by product source"
    ),
    sources: list[str] | None = Query(None, description="Filter tags by product source"),
    type: list[str] | None = Query(None, alias="type", description="Filter tags by product type"),
    types: list[str] | None = Query(None, description="Filter tags by product type"),
    search: str | None = Query(
        None, description="Case-insensitive substring filter on product name"
    ),
    ...
    tag_search: str | None = Query(
        None, description="Case-insensitive substring filter on tag name"
    ),
    limit: int | None = Query(None, le=1000, description="Optional cap; omit to return all tags"),
    ...
    try:
        ...
        if include_banned:
            if not current_user or current_user.get("role") not in {"admin", "moderator"}:
                raise HTTPException(
                    status_code=403,
                    detail="Moderator or admin role required to view banned products",
                )
        else:
...
async def get_products(
    ...
    sort_by: str = Query(
        "created_at",
        pattern="^(created_at|updated_at|rating)$",
        description="Sort by: created_at (default), updated_at, or rating",
    ),
    sort_order: str = Query(
        "desc", pattern="^(asc|desc)$", description="Sort order: desc (default) or asc"
    ),
...
async def count_products(
    ...
    try:
        count_query = _apply_product_filters(
            db.table("products").select("id", count="exact"), db, filters
        )
...
async def patch_product(
    ...
    try:
        ...
        result["editor_ids"] = (
            [editor["user_id"] for editor in editors_response.data] if editors_response.data else []
        )
    except Exception as e:
        logger = logging.getLogger(__name__)
        logger.error(f"[Editors] Failed to fetch editors for product {product_id}: {e}")
...
async def bulk_delete_products(
    ...
    try:
        ...
        print(
            f"[Bulk Delete] About to delete {len(ids_to_delete)} products: {ids_to_delete[:5]}..."
        )
        print("[Bulk Delete] Backend type: supabase")
            ...
            async with httpx.AsyncClient(base_url=settings.SUPABASE_URL, timeout=30) as client:
                for i in range(0, len(ids), chunk_size):
                    ...
                    if resp.status_code not in (200, 204):
                        ...
                        raise HTTPException(
                            status_code=500, detail="Failed to delete products in Supabase"
                        )
                    deleted += len(chunk)
            return deleted
        ...
        return {
            ...
            "source": next(iter(filters["source_values"]))
            if len(filters["source_values"]) == 1
            else None,
        }
    except Exception as e:
        logger = logging.getLogger(__name__)
...
async def ban_product(
    ...
    if not product_response.data and _looks_like_uuid(product_slug):
        product_response = (
            db.table("products").select("id").eq("id", product_slug).limit(1).execute()
        )
...
async def unban_product(
    ...
    updated = (
        db.table("products")
        .update({"banned": False, "banned_reason": None, "banned_by": None, "banned_at": None})
        .eq("id", product_id)
        .execute()
    )
